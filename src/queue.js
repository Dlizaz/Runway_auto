...
import { generateVideo, upscaleVideo } from "./runway.js";
...
async function processItem(item, options) {
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      ...
      const upscaleEnabled = item.upscale !== false && options.upscaleEnabled !== false;
      if (upscaleEnabled) {
        updateItemStatus(item.id, { state: "UPSCALING" });
        try {
          const upscaleTask = await upscaleVideo({
            videoUrl,
            resolution: options.upscaleResolution || "4k"
          });
          ...
          console.error(`Upscale ${item.id} that bai, dung ban goc:`, upscaleError.message);
          updateItemStatus(item.id, { upscaleError: upscaleError.message });
        }
      }
